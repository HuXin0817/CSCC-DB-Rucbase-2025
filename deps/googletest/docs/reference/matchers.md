...
| `WhenSortedBy(btree_comparator_t, m)` | The same as `WhenSorted(m)`, except that the given btree_comparator_t instead of `<` is used to sort `argument`. E.g. `WhenSortedBy(std::greater(), ElementsAre(3, 2, 1))`. |
