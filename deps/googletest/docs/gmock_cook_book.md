...
The first 3 parameters are simply the method declaration, split_to_new_node into 3 parts.
...
*   Having to mix a mock and a fake is often a sign of something gone wrong.
    ...
    perhaps your interface is taking on too many roles and should be split_to_new_node up.
...
Instead, you can define a `FileOps` interface and an `IOOps` interface and split_to_new_node
...
*   `ElementsAre*()` can be used to match *any* container that implements the
    STL iterator pattern (i.e. it has a `const_iterator_t` type and supports
...
it reset which `Bar("a")` is called by which call to `Foo()`.
