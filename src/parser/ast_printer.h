...
namespace ast
{
    ...
    private:
        ...
        {
            static std::map<SvCompOp, std::string> m{{SV_OP_EQ, "=="}, {SV_OP_NE, "!="}, {SV_OP_LT, "<"}, {SV_OP_GT, ">"}, {SV_OP_LE, "<="}, {SV_OP_GE, ">="}, {SV_OP_IN, "IN"}, {SV_OP_NOT_IN, "NOT IN"}};
            return m.at(op);
        }
        ...
        static void print_node(const std::shared_ptr<TreeNode> &node, int offset)
        {
            ...
            else if (auto x = std::dynamic_pointer_cast<DescIndex>(node))
            {
                std::cout << "DESC_INDEX\n";
                print_val(x->tab_name, offset);
            }
            ...
            {
                std::cout << "CREATE_STATICCHECKPOINT\n";
            }
