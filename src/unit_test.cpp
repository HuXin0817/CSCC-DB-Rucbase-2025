...
void check_equal(const RmFileHandle *file_handle, const std::unordered_map<Rid, std::string, rid_hash_t, rid_equal_t> &mock)
{
    // Test all records
    for (auto &entry : mock)
    {
        ...
        auto n = memcmp(mock_buf, rec->data, file_handle->file_hdr_.record_size);
        assert(n == 0);
    }
    // Randomly get record
    for (int i = 0; i < 10; i++)
    {
        Rid rid = {.page_no = 1 + rand() % (file_handle->file_hdr_.num_pages - 1), .slot_no = rand() % file_handle->file_hdr_.num_records_per_page};
    ...
    {
        auto n = mock.count(scan.rid()); // n = 0
        assert(n > 0);
...
std::ostream &operator<<(std::ostream &os, const Rid &rid) { return os << '(' << rid.page_no << ", " << rid.slot_no << ')'; }
...
TEST_F(BufferPoolManagerTest, SampleTest)
{
    ...
    EXPECT_EQ(false, page0->is_dirty_);
...
TEST(StorageTest, SimpleTest)
{
    ...
    for (auto &fh : mock)
    {
        ...
        for (page_id_t i = 0; i < MAX_PAGES; i++)
        {
            ...
            std::memcpy(page->get_data(), init_buf, PAGE_SIZE);
            buffer_pool_manager->unpin_page(PageId{fd, page_no}, true);

            char *mock_buf = mock_get_page(fd, page_no); // &mock[fd][page_no * PAGE_SIZE]
            std::memcpy(mock_buf, init_buf, PAGE_SIZE);
...
TEST(RecordManagerTest, SimpleTest)
{
    ...
    //    int record_size = RM_MAX_RECORD_SIZE ;  // 元组大小随便设置，只要不超过RM_MAX_RECORD_SIZE
    // test files
    {
        // 删除残留的同名文件
        if (disk_manager->is_file(filename))
        ...
        int max_bytes = file_handle->file_hdr_.record_size * file_handle->file_hdr_.num_records_per_page + file_handle->file_hdr_.bitmap_size + (int)sizeof(RmPageHdr);
        assert(max_bytes <= PAGE_SIZE); // BUG 1
        int rand_val = rand();
    ...
    for (int round = 0; round < 1000; round++)
    {
        ...
        check_equal(file_handle.get(), mock); // BUG
    }
