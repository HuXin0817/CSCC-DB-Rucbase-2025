...
public:
    ...
    /**
     * @description: 找第一个为0的位
     * @param {char*} bm 要找的起始地址为bm
     * @param {int} max_n 要找的位图的总位数
     * @return {int} 找到了就返回偏移位置，没找到就返回max_n
     */
    static int find_first_zero(const char *bm, int max_n) { return first_bit(false, bm, max_n); }
